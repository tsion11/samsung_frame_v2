...
export default function Art({ data }) {
  ...
  const filteredData = () => {
    ...
    return data.data?.[filterKey]?.attributes.arts.data
  }
    ...
    <>
      ...
      <div className="works-row  masonry -col-3 -gap-32 pt-60 sm:pt-40 js-masonry js-masonry-no-filter">
        ...
        {data && filteredData().map((d) => (
          <div key={d.id} className="masonry__item e_commerce">
            <div className="h:zoom ratio">
              <div className="absolute-full-center hover-group overflow-hidden rounded-8">
                <img
                  ...
                  src={`http://localhost:1337${d?.attributes?.pictures?.data[0].attributes.url}`}
                  alt="image"
                />
